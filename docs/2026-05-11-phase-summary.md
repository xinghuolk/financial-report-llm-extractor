...
| **R4 DB-aware `pipeline` command** | **已落地 (2026-05-13)** | R4 plan | New `pipeline` subcommand: pre-check R1 DB for (company, period, catalog_version) → return cache_hit if present; else run fetch (R2 cache) + evaluate (R3 cache) + auto-index. `--force` bypass. `evaluation.json` embeds `catalog_version` for accurate R1 indexer labels. **Phase R complete: 4 stages, two-level extraction cache (DB + filesystem caches at provider+LLM tiers) shipped.** |
| **R5 R1 schema market-scoping** | **已落地 (2026-05-14)** | R5 spec rev 4 prerequisite | field_values PK 加 market 列 (schema_version field_values_v2); init_db 检测+migration; query_extraction/query_field market 必选参数; CLI query --market required; pipeline 后置 patch (commit 6d195c5) 替换为 native filter. **Phase 1a (FinancialReportClient productization) prerequisite — 已 unblock**. 705 pytest passed. |
