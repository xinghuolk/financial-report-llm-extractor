...
- Modify `src/financial_report_llm_extractor/structured_sources/mapping.py`: tighten `statement_metadata_proven` so HK proof works for AKShare and Yahoo but rejects `unit == currency`.
...
- Test `tests/test_source_mapping.py`: HK metadata proof passes for valid AKShare/Yahoo candidates and fails when `unit == currency`.
...
Expected before implementation: the Yahoo proof test fails because `_statement_metadata_proven()` currently only returns true for AKShare HK, or the currency-as-unit test fails if current logic treats it as proven.

- [ ] **Step 3: Implement source-aware metadata proof**
...
git commit -m "fix: prove hk source statement metadata"
```
...
## Task 4: Expose Metadata Warnings In Provider Replay
...
Expected before implementation: `metadata_warning_fields` is missing from review output.

- [ ] **Step 3: Add metadata warning list to `_review_lists()`**

In `provider_baseline_replay.py`, add this list to `field_lists`:

```python
"metadata_warning_fields": sorted(
    field_id
...
```python
"  - metadata_warning_fields: "
f"{_format_field_list(review['metadata_warning_fields'])}",
```
...
Expected: replay tests pass and summary includes `metadata_warning_fields`.
...
- Type consistency: The plan reuses existing `SourceInventoryRecord`, `TurtleMappingCandidate.statement_metadata_proven`, `SourcePolicyItem`, and provider replay summary shapes.
