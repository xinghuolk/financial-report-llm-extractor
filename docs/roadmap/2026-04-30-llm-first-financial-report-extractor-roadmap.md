...
>
> Implementation status (2026-05-11): Phases A1–E (source-first foundation), Phase H0 (null_means_zero), Phase H1 (surgical conflict resolution; partially reverted post-review), Phases I-D/I-A/I-A.2 (LLM-assisted HK notes extraction with 6 follow-ups closed), Phases M2–M5 (HK terminal closure + provider semantics correction), Phases N0–N4 (catalog expansion 15 → 44 fields), Phase I-C (text-mode for 12 P3 pdf_only fields, total 56), Phase I-C.1 (whitespace-normalized retrieval), Phase EC (evaluate-company orchestrator for per-(company, period) regression validation), Phase H2 (CN/HK conflict surgical resolution), Phase H2.1 (CN SGA addition derivation), Phase H2.2 (multi-company sample-verification + market-scoped source_aliases + clean-row candidate audit display), Phase H2.3 #3 (persistent multi-company fixture + sample-vs-fixture regression), Phase HK-C (industry_not_applicable catalog override for 01113 SGA real-estate convention), Phase H2.4 (3 surgical fixes for cumulative review findings), Phase HK-LLM-2/C (regression-locked LLM supplement merge expanded to full 6 HK companies), Phase HK-B recon + HK-B.1-.4 shape locks, Phase MX (coverage matrix verification audit 24/62 → 36/62), Phase HK-B.5 (acct_payable PDF spot-check + per-issuer allowlist schema), Phase HK-B.5.1 (HK issuer financial-currency map at fetch time), Phase HK-B.5.2 (fixture backfill + trust policy multi-currency schema), Phase HK-B.5.3 (revenue + net_profit multi-currency PDF verification — mirage recovery +8 cells), Phase HK-B.6 (fix_assets 4-issuer PDF promotion), Phase HK-B.7 (5 BS aggregates extended to multi-currency), and Phase HK-B.8 (accounts_receiv 6-issuer promotion) — complete.
>
> **Coverage milestone (post-HK-B.8 multi-currency closure)**: CN 600519/2024 reaches **P0+P1 = 33/33 (100%) clean_present** after H2.2. Source-first 39/56 (70%) → **+LLM supplement 44/56 (79%)**, regression-locked by `tests/test_phase_hk_llm_2_supplement_merge.py`.
>
> **HK companies (regression-locked, post Phase HK-B.5 → .8)**: 00001/2025 source-first 31/56 (55%) → **+LLM 36/56 (64%)**; 01113/2025 source-first 32/56 (57%) → **+LLM 36/56 (64%)**; 01810/2024 (CNY reporter) source-first 33/56 (59%) → **+LLM 40/56 (71%)**; 02498/2024 (CNY) source-first 33/56 (59%) → **+LLM 38/56 (68%)**; 06862/2024 (CNY) source-first 34/56 (61%) → **+LLM 39/56 (70%)**; 09987/2024 (USD) source-first 32/56 (57%) → **+LLM 35/56 (63%)**. All 6 HK issuers now PDF-verified across multi-currency trust rules; `HK_ISSUER_FINANCIAL_CURRENCY` map + `additional_trusted_currencies` + `pdf_verified_company_ids` form the architectural closure. HK trust policy verified samples: **14 → 58 (4x growth)**. HK-B.4 `gross_profit` remains terminal_unverified per recon (no PDF anchor for HK GP); HK-B.6 01810/02498 `fix_assets` excluded from allowlist due to material PDF-vs-Yahoo Net PPE discrepancies, kept clean via single-source path.
>
> **Sample-verification breadth**: 4 CN companies × 4 fields = 16 EXACT-match samples backing H2/H2.1 CN promotions. 6 HK companies × 10 fields = **58 PDF-verified Yahoo HK trust samples** backing the multi-currency closure (post-HK-B.8). interest_paid_cash still single-sample (non-financial issuers don't report PAY_INTEREST_COMMISSION).
>
> Live LLM batch validation on 6 HK companies × 14 P3 fields: 33/84 (39%) present, 0 extraction_failed.
>
> **Next implementation slice (post-HK-B.8)**: candidates are CN trust rule multi-company extension (current CN has 4 issuers, can extend further); Yahoo HK adapter live-detect `financialCurrency` (replace hardcoded map); merge to main (branch is 320 commits ahead, all promotions PDF-evidenced). See `docs/2026-05-11-phase-summary.md` §6.
...
- **Yahoo HK + AKShare HK adapter currency-label fix** — *Phase HK-B.5.1 landed 2026-05-11*: `source_inventory_fetch.HK_ISSUER_FINANCIAL_CURRENCY` map stamps known HK issuers (00001/01113→HKD, 01810/02498/06862→CNY, 09987→USD) with their actual reporting currency. Used by both `_fetch_yahoo_for_company` and the CLI's AKShare `hk_default_currency` arg. Unknown HK issuers still fall back to HKD.
- **Phase HK-B.5.2 landed 2026-05-11**: fixture backfill rewrote 1616 records across 4 HK extension fixtures + multi-currency trust rule schema (`additional_trusted_currencies`) + 09987 re-added to acct_payable allowlist. All 6 HK issuers now promote acct_payable to clean_present at the correct currency. **Honest coverage adjustment**: backfill exposed that several other fields on non-HKD issuers (revenue, net_profit etc.) were "clean" only because the wrong-HKD-label was accidentally triggering HKD-scoped trust policies. Those now correctly revert to unresolved_conflict until separately PDF-verified for CNY/USD reporters. See `docs/phase_hk_b_5_recon.md` Phase HK-B.5.2 section.
- **Phase HK-B.5.3 landed 2026-05-11**: `revenue` and `net_profit` trust rules extended with `additional_trusted_currencies=["CNY","USD"]` + 8 new PDF-verified samples (4 non-HKD HK issuers × 2 fields). Recovers the 8 mirage cells lost during HK-B.5.2 backfill with honest provenance. Coverage: 01810 30→32, 02498 30→32, 06862 31→33, 09987 28→30.
- **Remaining multi-currency rule extensions (future)**: total_assets / total_liabilities / total_cur_assets / total_cur_liab / inventories / defer_tax_liab trust rules are still HKD-only. These survived the HK-B.5.2 backfill via the deterministic-candidate path (both providers agreed without needing trust policy), so they weren't mirage-affected. Extending them with multi-currency would primarily add explicit traceability rather than recover coverage.
- **Phase HK-B.6 landed 2026-05-11**: fix_assets trust rule added with `additional_trusted_currencies=["CNY","USD"]` + `pdf_verified_company_ids=["00001","01113","06862","09987"]`. PDF spot-check verified Yahoo Net PPE = PDF PP&E + Right-of-use assets (HKFRS 16 aggregation) for 4 of 6 HK issuers within rounding. 01810 (-22% material discrepancy) and 02498 (2.9% off) excluded from allowlist; they stay clean via single-source path (AKShare returns None for 固定资产 on these issuers). Coverage: 00001 / 01113 / 09987 fix_assets unresolved_conflict → clean_present (+3 cells). 06862 already clean via single source; now also documented via trust rule.
- **Phase HK-B.7 landed 2026-05-11**: 5 remaining HK trust rules extended with `additional_trusted_currencies=["CNY","USD"]` + 4-issuer non-HKD samples each (20 new samples total) — `total_assets`, `total_liabilities`, `total_cur_assets`, `total_cur_liab`, `inventories`. All 20 PDF spot-checks pass exact (no rounding ambiguity, these are simple BS aggregates). These fields survived HK-B.5.2 backfill via deterministic-candidate path so coverage didn't change, but now the trust policy has explicit per-issuer PDF-verified evidence for traceability + future-proof against AKShare/Yahoo divergence on these aggregates. Trust policy verified samples: 32 → 52. `defer_tax_liab` skipped — Yahoo Deferred Tax Liabilities Non Current not reported for any of the 4 non-HKD issuers.
- **Phase HK-B.8 landed 2026-05-11**: `accounts_receiv` promoted from 6-conflict shape (HK-B.3 lock) to 6-clean via multi-currency Yahoo trust rule. PDF spot-check across all 6 HK issuers confirmed Yahoo `Accounts Receivable` = PDF pure Trade Receivables (HK property-co convention: 'Debtors'; Yum China: 'Accounts receivable, net'). Subtle: Yum China Yahoo has BOTH a broader 'Receivables' raw_field ($316M including payment-processor receivables) AND the canonical 'Accounts Receivable' ($79M matching PDF exactly) — the source mapping correctly picks the pure-AR alias. Coverage: +6 cells (one per HK issuer). Trust policy samples: 52 → 58. `gross_profit` deliberately not promoted — recon (`docs/phase_hk_b_recon.md`) confirmed HK formal income statements lack a gross profit row; Yahoo/AKShare both compute via different methods with no PDF anchor, drift §177 advises against provider-provider agreement alone.
