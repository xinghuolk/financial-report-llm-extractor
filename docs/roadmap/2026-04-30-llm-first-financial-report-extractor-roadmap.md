...
- **Multi-currency rule extension (future)**: revenue / net_profit / total_assets / total_liabilities / inventories / defer_tax_liab / total_cur_assets / total_cur_liab trust rules are still HKD-only. Extending them with `additional_trusted_currencies = ["CNY","USD"]` requires per-issuer PDF spot-check (drift §177); deferring until a structured iteration.
