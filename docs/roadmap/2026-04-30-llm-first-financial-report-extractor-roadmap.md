...
- **Yahoo HK adapter currency-label adapter fix**: `source_inventory_fetch.py` hardcodes `currency=HKD` for every HK ticker, so RMB-issuer (Xiaomi/Anta/06862) and USD-issuer (Yum China) inventory records carry the wrong currency label. The HK-B.5 `pdf_verified_company_ids` allowlist excludes 09987 from clean promotion to avoid emitting a wrong-currency claim, but doesn't fix the underlying RMB-mislabel problem inherited from the pre-HK-B.5 baseline (01810/02498/06862 acct_payable were already clean via akshare with the same wrong HKD label). A future phase should detect issuer reporting currency in the Yahoo HK adapter and stamp inventory accordingly. AKShare HK adapter likely has the same gap.
