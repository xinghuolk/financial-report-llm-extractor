...
| R5 | `cache/db_schema.py` field_values_v2 + `cache/db.py` init_db migration + `cache/indexer.py` market-scoped DELETE+INSERT + `cache/db_query.py` market required param + `cli.py` query --market required | R1 schema v2 fix (Phase 1a client API prerequisite): field_values PK 加入 market 列 (was 3-col, now 4-col); `init_db()` 检测 v1 schema → drop + recreate（接受重 index 成本，tmp/runs 是 source of truth）; query_extraction/query_field 加 market 必选参数; CLI `query --market {CN,HK}` 必选; pipeline 删除冗余 post-hoc market check (commit 6d195c5 patch 替换为 native filter)。同 (company, period_end) 不同 market 共存不互覆盖。Operator 须 `rm data/extracted.db && financial-report-llm-extractor index --runs tmp/runs ...` 重 index。 |
